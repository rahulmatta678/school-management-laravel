<x-app-layout>
    <div class="mb-12 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
            <h2 class="text-4xl font-black text-white uppercase italic tracking-tighter leading-none">
                {{ __('Student registry') }}<span class="text-indigo-500">.</span>
            </h2>
            <p class="text-slate-500 mt-2 font-bold uppercase text-[10px] tracking-[0.2em]">Manage academic profiles and enrollment data</p>
        </div>
        <a href="{{ route('teacher.students.create') }}" class="group flex items-center px-8 py-4 bg-white text-black font-black text-xs uppercase italic tracking-widest rounded-full hover:bg-indigo-600 hover:text-white transition-all duration-300 shadow-xl active:scale-95">
    ...
    <x-data-table :headers="['Student Detail', 'Credentials', 'Classification', 'Actions']" :items="$students">
        @forelse ($students as $student)
            <tr class="elite-hover group">
                ...
                <td class="px-8 py-6 whitespace-nowrap">
                    <div class="text-xs font-black text-slate-300 lowercase italic">{{ $student->email }}</div>
                ...
                <td class="px-8 py-6 whitespace-nowrap text-[9px] text-slate-500 font-black uppercase tracking-widest italic">
                     <span class="inline-flex items-center px-4 py-1.5 rounded-xl bg-white/5 text-slate-400 border border-white/5">
                        {{ $student->grade ?? 'NO GRADE' }} / {{ $student->age ?? '??' }}Y
                    </span>
                </td>
                <td class="px-8 py-6 whitespace-nowrap text-right text-xs font-black uppercase italic">
                    <div class="flex items-center justify-end gap-6 text-[10px] tracking-widest">
                        <a href="{{ route('teacher.students.edit', $student) }}" class="text-slate-500 hover:text-white transition-colors flex items-center gap-2">
                        ...
                        <button type="button" x-data="" x-on:click="$dispatch('open-modal', 'confirm-delete-{{ $student->id }}')" class="text-slate-600 hover:text-rose-500 transition-colors flex items-center gap-2">
        ...
        @empty
            <tr><td colspan="4" class="px-8 py-20 text-center text-[10px] font-bold text-slate-600 uppercase tracking-widest italic">No student records found</td></tr>
        @endforelse
    </x-data-table>
    ...
    @foreach ($students as $student)
        <x-delete-confirmation 
            ...
            title="Deauthorize Student"
            :message="'Are you sure you want to permanently remove ' . $student->name . ' from the registry?'"
        />
    @endforeach
</x-app-layout>
